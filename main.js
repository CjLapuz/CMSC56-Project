...
function getComputerMove(boardStateOrig) {
    ...
    return alphaBeta(boardStateOrig, -1000000, 1000000, 0, -1, true);
}

// alphaBeta algorithm

function alphaBeta(boardStateOrig, alpha, beta, depth, color, maximizingPlayer) {
    ...
    if (checkWin(RED, boardState) === WON) {
        ...
        rows: for (var x1 = 0; x1 < MAX_X; x1++) {
            ...
            if (depth < MAX_DEPTH) {
                ...
                val = -1000000;
                val = max(val,alphaBeta(boardState2, -1000000, 1000000, depth + 1, color * -1, true);
                alpha = max(alpha,val);
                if(beta <= alpha){
                    break;   
                }
                    ...
                    val = 1000000;
                    val = max(val, alphaBeta(boardState2, -1000000, 1000000, depth + 1, color * -1, false);
                    if(beta <= alpha){
                        break;
                    }
                    return val;
